---
layout: post
title:  "ARGB Gen2 - Protocol Analysis"
date:   2024-01-21 16:30:02 +0200
categories: RGB LED, reverse engineering, ARGB Gen2
---

### Table of Contents
- [Introduction](#introduction)
- [The companies behind ARGB Gen2](#the-companies-behind-argb-gen2)
- [Investigated Hardware](#investigated-hardware)
- [ARGB Gen2 vs WS2812](#argb-gen2-vs-ws2812)
...
# The companies behind ARGB Gen2
...
Since I was not able to locate any discrete ARGB Gen2 LEDs, let alone find data-sheets. I purchased a LED controller ([Coolermaster A1](https://www.coolermaster.com/catalog/coolers/rgb-accessories/led-controller-a1/)) and a fan with ARGB Gen2 LEDs ([Artic P12 PWM PST-ARGB](https://www.arctic.de/en/P12-PWM-PST-A-RGB/ACFAN00231A)). 

![Devices](../_images/ARGB2_devices.jpg)

The LED controller is a USB device that can be connected to a PC and controlled via software. Up to three ARGB Gen2 LED strings can be connected to it. The controller contains a single custom IC, most likely a micro-controller.
...
To implement these additional functionality, the WS2812 protocol is extended in the ARGB GEN2 devices. This is not supported in typical WS2812 controllers and may require software and potentially even hardware changes to support it.
...
Since the WS2812 protocol does not allows to transmit additional commands, the ARGB GEN2 designers had to find a way to "escape" from the defined code sequence. The only way of doing this is to use signal timings that are outside of the WS2812 specification. In this case they use combinations of long pulses to indicate special commands.
...
Below you can find example code that can be used to send configuration data to ARGB GEN2 LEDs. The code is based on the light ws librariy on an ATMega328p and uses PB0 for communication.
...
The oscilloscope image above shows the timing of the sequence. Note that the string does not react to the second command because it already had an address assigned (It will also not respond to other address assignment commands). Also note, that the commands are not forwarded to the second device in the string. Only the "gateway device" takes part in the address assignment procedure.
...
The ARGB Gen2 represents a notable improvement over the vanilla WS2812, introducing features such as bidirectional communication, configuration mode, and Y-Cable mode. These enhancements significantly expand the capabilities of the classic WS2812 protocol, addressing some of its inherent limitations. 

However, the ARGB GEN2 extensions to the WS2812 protocol are also quite purpose defined and do not offer hooks for additional features. For example, adding more configuration options would require an additional protocol extension.

It seems that ARGB GEN2 does not yet have caught on outside of products for enthusiast PC builders. However, the availability of the SK6112 LEDs provides access to at least some of the features in discrete LEDs.
